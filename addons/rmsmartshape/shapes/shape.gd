...
@export_group("Light")
##LIGHT OCLUDER2D SE ESTA INTENTANDO IMPLEMENTAR
@export_node_path("LightOccluder2D")
var light_occluder_node_path: NodePath : set = set_light_occluder_node_path
...
func bake_light_occluder() -> void:
	...
	# Respeta el mismo modo de actualización que colisión
	if collision_update_mode == CollisionUpdateMode.Editor and not Engine.is_editor_hint():
		return
	if collision_update_mode == CollisionUpdateMode.Runtime and Engine.is_editor_hint():
		return

	if _points.get_point_count() < 2:
		if _light_occluder_node.occluder:
			_light_occluder_node.occluder.polygon = PackedVector2Array()
		return

	# Asegurar que exista el recurso
	if not _light_occluder_node.occluder:
		_light_occluder_node.occluder = OccluderPolygon2D.new()
	...
	# MISMO XFORM que colisiones (CRÍTICO)
	var xform := _light_occluder_node.get_global_transform().affine_inverse() * get_global_transform()
	generated_points = xform * generated_points

	# Evitar punto duplicado final
	if generated_points.size() > 1 and generated_points[0] == generated_points[-1]:
		generated_points.resize(generated_points.size() - 1)

	_light_occluder_node.occluder.polygon = generated_points
